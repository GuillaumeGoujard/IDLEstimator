...
fenchels = [0.1, 0.001, 0.0001, 0.00001 ]
results = {}
f = fenchels[2]
hidden_variables = [1, 10, 100, 500, 1000]
for h in hidden_variables:
    IDL = idl.IDLModel(hidden_variables=h, dual_learning_rate=dual_learning_rate, tol_fenchtel=f,
                       random_state=0, verbosity=True, solver="SCS", inner_tol=1e-6)
    IDL.fit(X_train, y_train, rounds_number=500, verbose=True, type_of_training="two_loops")
